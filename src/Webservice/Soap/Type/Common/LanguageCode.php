...
class LanguageCode
{
     ...
     * Constructor.
     *
     * @param string|null $value ISO3 Language Code
     */
    public function __construct(?string $value = self::__DEFAULT)
    {
        if (empty($value)) {
        ...
        if (\strlen($value) == 2) {
            $value = self::isoToDhlLanguageCode($value);
        }

        if (!in_array($value, self::$supportedLanguageCodes)) {
            throw new \InvalidArgumentException(
                'Invalid argument for ' . get_class($this) . ': Language code ' . $value . ' is not supported'
            );
        }
